...
func teardownDDBTestTbl(ctx context.Context, client dynamodbiface.DynamoDBAPI, tableName string) {
...
func (c *controller) handlerFunc(w http.ResponseWriter, r *http.Request) {
	...
	defer teardownDDBTestTbl(ctx, c.ddbClient, ddbTblName)
...
func main() {
	...
	srv := http.Server{
		Handler: http.HandlerFunc(c.handlerFunc),
		Addr:    ":8080",
	}
